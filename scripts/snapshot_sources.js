...
const UA='central-missing-person-resolution/1.2 evidence-snapshot';
...
function curl(url,insecure=false){
  const args=['-fsSL','--retry','3','--retry-all-errors','--connect-timeout','15','--max-time','90','-A',UA];
...
async function fetchText(url){
  ...
  for(let i=0;i<3;i++){
    try{
      ...
      if(r.status>=200&&r.status<400&&body) return {status:r.status,body,transport:'fetch',tls_verification:'verified'};
      last=`HTTP ${r.status}`;
    }catch(e){last=String(e)}
    await sleep(600*(i+1));
  }
  try{
    const body=curl(url,false);
    if(body) return {status:200,body,transport:'curl',tls_verification:'verified'};
  }catch(e){last=`${last}; curl verified: ${e.message}`}
  if(isNepalPolice(url)){
    try{
      const body=curl(url,true);
      if(body) return {status:200,body,transport:'curl-insecure-fallback',tls_verification:'BYPASSED_AFTER_VERIFIED_TLS_FAILURE'};
    }catch(e){last=`${last}; curl insecure fallback: ${e.message}`}
  }
...
function discoverPages(html){
  ...
  return Math.min(Math.max(max,1),250);
}
(async()=>{
  const state=[];
  for(const s of manifest.sources){
    ...
    try{
      ...
      const item={source_id:s.id,url:s.url,retrieved_at:started,http_status:first.status,sha256:base.sha256,bytes:base.bytes,changed:base.changed,error:null,transport:first.transport,tls_verification:first.tls_verification,pages:1,page_failures:[]};
      if(s.pagination==='discover_all'){
        ...
        for(let p=2;p<=maxPage;p++){
          ...
          try{
            ...
            if(got.tls_verification!==item.tls_verification)item.tls_verification='MIXED';
  ...
  const hard=state.filter(x=>x.critical&&(x.error||x.page_failures.length));
  if(hard.length){console.error('Critical source failures:',hard.map(x=>x.source_id).join(', '));process.exitCode=2}
})();
