...
fs.writeFileSync('data/update-meta.json',JSON.stringify({incident_id:INCIDENT,...version,record_count:records.length,unidentified_body_record_count:bodies.length,source_counts,status_counts},null,2)+'\n');
console.log(JSON.stringify({incident_id:INCIDENT,dataset_version,generated_at,records:records.length,bodies:bodies.length,source_counts,status_counts,identity_collisions:collisions.length},null,2));
